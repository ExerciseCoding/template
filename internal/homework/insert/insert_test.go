...
//	user := User{Name: "kellen", Address: "shanxi"}
//
...
func TestInsertStmt(t *testing.T) {
	...
	for _, tc := range testCases {
		actualSql,actualArgs, err := InsertStmt(tc.entity)
		assert.Equal(t, tc.wantErr, err)
		if tc.wantErr != nil {
			// 预期会有错误返回，就不需要进一步校验其他两个返回值了
			return
		}
		// assert和require核心就是想不想后面的代码继续执行
		assert.Equal(t, tc.wantSQL, actualSql)
		assert.Equal(t, tc.wantArgs, actualArgs)
	}
}
